...
The injector waits for that idle prompt rather than guessing at a fixed delay. It samples the claude process's CPU time from `/proc/<pid>/stat` twice a second and fires only once the process has been idle across three consecutive samples, roughly a second and a half of quiet. It fires exactly once and then exits, and it gives up after ninety seconds if no idle window ever opens. The single-shot design matters: an injector that retried would queue several `/compact` invocations behind a busy prompt and spray them the moment it went idle.
